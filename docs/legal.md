# Legal information

© Max R. P. Grossmann, Holger Gerhardt, et al., 2026.

## What is uproot?

uproot is an open-source framework for developing and conducting browser-based behavioral experiments. It is designed for researchers who need to run studies with human participants, from simple surveys to complex real-time multiplayer interactions.

## Who created uproot?

uproot was originally developed by [Dr Max Grossmann](https://max.pm/) and [Dr Holger Gerhardt](https://www.econ.uni-bonn.de/iame/en/team/gerhardt). We are academic researchers who were frustrated by existing solutions for running behavioral experiments and decided to build something better.

We do not sell anything. We do not have big funders. uproot is Free/Libre Open Source Software under licenses approved by both the OSF and the FSF and will always remain so. uproot is [open to contributions](https://github.com/mrpg/uproot).

## License

uproot is licensed under the [GNU Lesser General Public License v3.0](https://www.gnu.org/licenses/lgpl-3.0.en.html) (LGPL-3.0-or-later). This means you can use uproot freely in your research, including in proprietary projects, as long as modifications to uproot itself are shared under the same license.

The documentation and code examples are licensed under the [0BSD license](https://opensource.org/licenses/0BSD), allowing unrestricted use without attribution.

## Warranty disclaimer

uproot is provided "as is", without warranty of any kind, express or implied. To the extent permitted by Law, we disclaim all warranties, including but not limited to fitness for a particular purpose and non-infringement. **Use uproot at your own risk.**

## Contact

[Dr Grossmann](https://max.pm/contact/), [Dr Gerhardt](https://www.econ.uni-bonn.de/iame/en/team/gerhardt).
