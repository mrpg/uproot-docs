...
- `your_app/AdminSettings.html`: This form is used for every config that includes `your_app`. If a config contains several apps with such a file, all their forms are shown, stacked in the config’s app order.
- `AdminSettings.html` at the project root: This is a project-level form that takes over for *all* configs; app-level fragments are then ignored.
...
- `config`: the selected config’s name; and
