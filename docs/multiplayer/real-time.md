...
Sometimes you need work to continue after a `@live` method returns. Examples are calling an external API or running a loop that pushes updates every few seconds. `spawn` launches an async function in the background so it keeps running independently:
